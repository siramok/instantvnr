...
namespace vnr
{
  ...
  public:
    ...
    void test(float *loss)
    {
      ...
      util::parallel_for_gpu(0, stream, m_batch_size,
                             [target = (float *)m_trainer.m_test_y0->data(),
                              ...
                              output = m_trainer.m_loss_buffer.data()] __device__(size_t i)
                             {
    ...
    void progressively_decode()
    {
      ...
      b++;
      if (b * m_trainer.m_num_slices_per_blob >= dims.z)
        b = 0;
    }
    ...
    void decode_volume(float *output, vec3i dims) const
    {
      ...
      for (int z = 0; z < dims.z; ++z)
      {
        ...
        slice_value.copy_to_host(output, count);
        output += count;
    ...
    void save_reference_volume(std::string filename, vec3i dims) const
    {
      ...
      if (!ofile)
        throw std::runtime_error("Cannot open file: reference.bin");
      ...
      ofile.close();
      if (!ofile.good())
        throw std::runtime_error("Error occurred at writing reference.bin");
    ...
    float get_mse(vec3i dims, bool quiet) const
    {
      ...
      for (int z = 0; z < dims.z; z += batch.z)
      {
        for (int y = 0; y < dims.y; y += batch.y)
        {
          ...
          if (!quiet)
            bar.update((float)(z * dims.y + y) / (dims.y * dims.z));
        }
        if (!quiet)
          bar.update((float)z / dims.z);
      }
    ...
    vec2f get_macrocell_psnr() const
    {
      ...
      util::parallel_for_gpu(0, 0, n_mcs,
                             [mc_pred = m_macrocell.d_value_range(),
                              ...
                              out_pred = pred.data(),
                              out_targ = targ.data()] __device__(size_t i)
                             {
  ...
  NeuralVolume::save_params_to_json(json &root) const
  {
    ...
    root["volume"] = {
        {"dims", {{"x", pimpl->m_dims.x}, {"y", pimpl->m_dims.y}, {"z", pimpl->m_dims.z}}}};
    root["macrocell"] = {
  ...
  void NeuralVolume::load_params_from_json(const json &root)
  {
    const auto &mc = pimpl->m_macrocell;
    ...
    if (root.contains("macrocell"))
    {
      ...
      {
        logging() << "[warning] mismatch macrocell dimension or spacing. Updating macrocell dims to "
                  << mcdims << " and spacings to " << mcspac << std::endl;
    ...
    {
      logging() << std::endl
                << "[network] reset model as: " << root["model"].dump(2) << std::endl;
  ...
  int NeuralVolume::get_network_width() const
  {
    return pimpl->m_neural->n_neurons();
  }

  int NeuralVolume::get_network_features_per_level() const
  {
