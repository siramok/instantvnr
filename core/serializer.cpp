...
#define define_scalar_serialization(T) \
  template <>                          \
  inline T scalar_from_json<T>(const json &in) { return in.get<T>(); }
    ...
    valid_filename(const json &in, const std::string &key)
    {
      if (in.contains(key))
      {
        ...
        if (js.is_array())
        {
          for (auto &s : js)
          {
            if (file_exists_test(s.get<std::string>()))
  ...
  create_scene_vidi__tfn(json jstfn, json jsvolume, ValueType type)
  {
    ...
    if (alpha[0].y < 0.01f)
      alpha[0].y = 0.f;
    if (alpha[tf.resolution() - 1].y < 0.01f)
      alpha[tf.resolution() - 1].y = 0.f;
  ...
  void create_json_tfn_stringify_vidi(json root, TransferFunction &tfn)
  {
    // directly parse transfer function and scalar mapping range
    assert_throw(root.contains("view"), "expected 'view' in TFN JSON");
    auto &view = root["view"];
    assert_throw(view.contains("volume"), "expected 'volume' in TFN JSON");
    auto &volume = view["volume"];
    assert_throw(volume.contains("transferFunction"), "expected 'transferFunction' in TFN JSON");

    auto &jstfn = volume["transferFunction"];

    tfn::TransferFunctionCore tf;
    tfn::loadTransferFunction(jstfn, tf);

    // finalize transfer function
    auto *table = (vec4f *)tf.data();
    std::vector<vec3f> color(tf.resolution());
    std::vector<vec2f> alpha(tf.resolution());
    ...
    // Set scalar mapping range directly
    if (volume.contains("scalarMappingRange"))
    {
      auto r = rangeFromJson(volume["scalarMappingRange"]);
      tfn.range.lower = r.x;
      tfn.range.upper = r.y;
    }
    else
    {
      throw std::runtime_error("missing 'scalarMappingRange' in TFN JSON");
    }
  }
  ...
  create_json_volume_stringify(json root, MultiVolume &volume)
  {
    ...
    {
      if (root["version"] == "DIVA")
        return create_json_volume_stringify_diva(root, volume);
      else if (root["version"] == "VIDI3D")
      {
      }
      else
